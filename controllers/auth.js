...
exports.getMyAccount = asyncHandler(async (req, res, next) => {
    const user = await User.findById(req.user.id).populate("interests.category");
    if (!user) {
...
//@route  POST /api/v1/auth/profile/:id
//@access Public
...
exports.register = asyncHandler(async (req, res, next) => {
    ...
    if (userCheck) {
        return next(new ErrorResponse(`${authMessages.registerAr}`, 400));
    }
    ...
    try {
        ...
        console.log(error);
        return next(
            new ErrorResponse(`${authMessages.verificationEmailNotSentAr}`, 500)
        );
    }
